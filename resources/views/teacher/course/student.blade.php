...
@section('hiddenSearch')
    ...
    <input type="hidden" name="class" value="{{is_null($class_key)?"":$class_key->id}}"/>
@endsection
...
@section('search')
    <tr>
        ...
        <td>
            <label>学生班级：</label>
            <input name="class.id" value="{{is_null($class_key)?"":$class_key->id}}" type="hidden"/>
            <input name="class.info" type="text" lookupGroup="class"
                   value="{{is_null($class_key)?"":($class_key->grade.$class_key->Major->name.$class_key->num."班")}}"/>
...
@section('buttons')
    ...
    <li><a class="edit" href="{{url("/student/edit")}}?id={id}" target="dialog" rel="studentEdit" width="1280" height="800"><span>修改</span></a>
    </li>
